...
function injector() {
    ...
    (function(GM_info) {
        ...
        $ {
            scriptSource.name
        }('${GM_info.scriptHandler}.${injector.name}')
    })($ {
...
function scriptSource(invokeBy) {
    ...
    const r = {
        ...
        baipiao: [{
                ...
                match: () => (window.__INITIAL_STATE__ && window.__INITIAL_STATE__.epInfo && window.__INITIAL_STATE__.epInfo.ep_id) === 251255,
                link: 'http://www.acfun.cn/bangumi/ab5022161_31405_278830',
                message: r_text.welcome_to_acfun
            }, {
                key: 'zomble_land_saga',
                match: () => (window.__INITIAL_STATE__ && window.__INITIAL_STATE__.mediaInfo && window.__INITIAL_STATE__.mediaInfo.media_id) === 140772,
    ...
    const util_log = util_log_impl('log') const util_info = util_log_impl('info') const util_debug = util_log_impl('debug') const util_warn = util_log_impl('warn') const util_error = util_log_impl('error') log = util_debug log(`[$ {
            GM_info.script.name
        }
        v$ {
    ...
    /** 通知模块 剽窃自 YAWF 用户脚本 硬广：https://tiansh.github.io/yawf/ */const util_notify = (function() {
        var avaliable = {};
        var shown = [];
        var use = {
        ...
        const loop = (time) => {
            if (!condition()) {
                if (time < retryCount) {
                    ...
                    return Promise.reject(`util_promise_condition timeout, condition: $ {
                        condition.toString()
                    }`)
                }
            } else {
    ...
    const _ = util_ui_element_creator const util_jsonp = function(url, callback) {
        return new Promise((resolve, reject) => {
        ...
        if (document.getElementById('AHP_Notice_style') == null) {
            ...
            document.head.appendChild(_('style', {
                id: 'AHP_Notice_style'
                    ...
                    margin: 2px;
                    padding: 4px;
                    color: #1E90FF;font-size:14px;font-weight:bold;border:# 1E90FF 2px solid;
                ...
                AHP_Notice > div > div {
                    ...
                    background: #EFEFF4;
                    color: #000;border-radius:5px;box-shadow:0 0 5px -2px;pointer-events:auto;white-space:pre-wrap } # AHP_Notice > div > div * {
                        margin: 5px 0;
                    }#
                    ...
                    AHP_Notice input[type = text]: active {
                        border - bottom - color: #4285f4 } # AHP_Notice input[type = button] {
                            border - radius: 2px;
            ...
            childs.push(_('input', {
                ...
                event: {
                    ...
                    }, childs)]))])); document.body.appendChild(div); div.style.height = div.firstChild.offsetHeight + 'px';
            }
            ...
            const balh_api_plus_playurl = function(cid, qn = 16, bangumi = true) {
                ...
                /BPplayurl.php?otype=json&cid=${cid}${bangumi ? '&module=bangumi' : ''}&qn=${qn}&src=vupload&vid=vupload_${cid}`)
    }
    / / https: //www.biliplus.com/api/h5play.php?tid=33&cid=31166258&type=vupload&vid=vupload_31166258&bangumi=1
                const balh_api_plus_playurl_for_mp4 = (cid, bangumi = true) => util_ajax(`$ {
                    balh_config.server
                }
                /api/h5play.php ? tid = 33 & cid = $ {
                    cid
                } & type = vupload & vid = vupload_$ {
                    cid
                } & bangumi = $ {
                    bangumi ? 1 : 0
                }`)
    ...
    const balh_feature_area_limit_new = (function () {
        ...
        function replacePlayInfo() {
            ...
            / / 将__playinfo__置空,
                                ...
                                replaceInitialState() replaceUserState() replacePlayInfo()
                                })()
                                                    ...
                                                    if (prop === 'onreadystatechange') {
                                                        ...
                                                        value = function(event) {
                                                            if (target.readyState === 4) {
                                                                if (target.responseURL.match(util_regex_url('bangumi.bilibili.com/view/web_api/season/user/status')) || target.responseURL.match(util_regex_url('api.bilibili.com/pgc/view/web/season/user/status'))) {
                                                                    ...
                                                                    if (json.code === 0 && json.data && balh_config.blocked_vip && !isFromReport // report时, 还是不伪装了...
                                                                    ) {
                                                                    ...
                                                                    if (balh_config.blocked_vip) {
                                                                        ...
                                                                        const xml = new DOMParser().parseFromString(` < root > $ {
                                                                            target.responseText.replace(/\&/g, "&amp;")
                                                                        } < /root>`, 'text/xml
                                                                        ')
                                                const vipXml = xml.querySelector('
                                                                        vip ')
                                                if (vipXml) {
                            ...
                            if (typeof value === 'function ') {
                                ...
                                value = function () {
                                    if (prop === '
                                        ...
                                        if (container.__url.match(util_regex_url('
                                                                        api.bilibili.com / x / player / playurl ')) && balh_config.enable_in_av) {
                                            log(' / x / player / playurl ')
                                            // debugger
                                        ...
                                        } else if (container.__url.match(util_regex_url('
                                                                        api.bilibili.com / pgc / player / web / playurl '))
                                            && !util_url_param(container.__url, '
                                                                        balh_ajax ')
                                            && needRedirect()) {
                                            log(' / pgc / player / web / playurl ')
                                            // debugger
        ...
        function injectAjax() {
            ...
            $.ajax = function (arg0, arg1) {
                ...
                if (param.url.match(util_regex_url_path(' / web_api / get_source '))) {
                    ...
                    || param.url.match(util_regex_url('
                                                                        api.bilibili.com / pgc / player / web / playurl ')) // 新的番剧页面playurl接口
                    || (balh_config.enable_in_av && param.url.match(util_regex_url('
                                                                        interface.bilibili.com / v2 / playurl '))) // 普通的av页面playurl接口
                ) {
                    // 新playrul:
                    ...
                    // 3. 失败时, 返回的结果没变
                    let isNewPlayurl
                    if (isNewPlayurl = param.url.match(util_regex_url('
                                                                        api.bilibili.com / pgc / player / web / playurl '))) {
                        if (param.data) {
                    ...
                    dispatchResultTransformer = p => p
                        .then(r => {
                            if (!r.code && !r.from && !r.result && !r.accept_description) {
                                util_warn('
                                                                        playurl的 result缺少必要的字段: ', r)
                                r.from = '
                                                                        local '
                                r.result = '
                                                                        suee '
                                r.accept_description = ['未知
                                                                        3P ']
                                // r.timelength = r.durl.map(it => it.length).reduce((a, b) => a + b, 0)
                                if (r.durl && r.durl[0] && r.durl[0].url.includes('
                                                                        video - sg.biliplus.com ')) {
                                    const aid = window.__INITIAL_STATE__ && window.__INITIAL_STATE__.aid || window.__INITIAL_STATE__.epInfo && window.__INITIAL_STATE__.epInfo.aid || '
                                                                        fuck '
                                    util_ui_pop({
                ...
                } else if (param.url.match(util_regex_url('
                                                                        interface.bilibili.com / player ? '))) {
                    if (balh_config.blocked_vip) {
                        mySuccess = function (data) {
                            try {
                                let xml = new window.DOMParser().parseFromString(`<userstatus>${data.replace(/\&/g, ' & amp;
                                                                        ')}</userstatus>`, '
                                                                        text / xml ');
                                let vipTag = xml.querySelector('
                                                                        vip ');
                                if (vipTag) {
        ...
        function setAreaLimitSeason(limit) {
            ...
            util_cookie.set('
                                                                        balh_season_ ' + season_id, limit ? '
                                                                        1 ' : undefined, '
                                                                        '); // 第三个参数为'
                                                                        ', 表示时Session类型的cookie
            log('
                                                                        setAreaLimitSeason ', season_id, limit);
        }
        ...
        function isBangumiPage() {
            return isBangumi(util_safe_get('
                                                                        window.__INITIAL_STATE__.mediaInfo.season_type || window.__INITIAL_STATE__.mediaInfo.ssType '))
        }
        ...
        function getSeasonId() {
            ...
            if (!seasonId) {
                try {
                    seasonId = (window.top.location.pathname.match(/\/anime\/(\d+)/) || ['
                                                                        ', '
                                                                        '])[1];
                } catch (e) {
                    log(e);
            ...
            if (!seasonId) {
                try {
                    seasonId = (window.top.location.pathname.match(/\/movie\/(\d+)/) || ['
                                                                        ', '
                                                                        '])[1];
                    if (seasonId) {
                        seasonId = '
            ...
            if (!seasonId) {
                try {
                    seasonId = (window.top.location.pathname.match(/\/bangumi\/play\/((ep|ss)\d+)/) || ['
                                                                        ', '
                                                                        '])[1];
                } catch (e) {
                    log(e);
            ...
            if (!seasonId) {
                try {
                    seasonId = (window.top.location.pathname.match(/\/video\/(av\d+)/) || ['
                                                                        ', '
                                                                        '])[1]
                } catch (e) {
                    log(e);
            ...
            if (!seasonId) {
                seasonId = util_url_param(window.location.href, '
                                                                        aid ');
                if (seasonId) {
                    seasonId = '
                                                                        aid ' + seasonId;
        ...
        function isAreaLimitForPlayUrl(json) {
            return (json.errorcid && json.errorcid == '
                                                                        8986943 ') || (json.durl && json.durl.length === 1 && json.durl[0].length === 15126 && json.durl[0].size === 124627);
        }
        ...
        var bilibiliApis = (function () {
            ...
            BilibiliApi.prototype.asyncAjaxByProxy = function (originUrl, success, error) {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        // log('
                                                                        success ', arguments, this);
                    },
                    error: function (e) {
                        log('
                                                                        error ', arguments, this);
                        error(e);
                    }
                });
            };
            ...
            var playurl_by_bilibili = new BilibiliApi({
                ...
                transToProxyUrl: function (originUrl) {
                    const api_url = '
                                                                        https : //interface.bilibili.com/playurl?'
                                                                        ...
                                                                        let useJson = false let paramDict = {
                                                                        ...
                                                                        let {
                                                                            sign, params
                                                                        } = util_generate_sign(paramDict, module ? SEC_BANGUMI : SEC_NORMAL) let url = module ? bangumi_api_url : api_url + params + '&sign=' + sign
                                                                        return url
                                                                        },
                                                                        processProxySuccess: function(result, alertWhenError = true) {
                                                                            ...
                                                                            if (!obj || obj.code) {
                                                                                if (alertWhenError) {
                                                                                    util_ui_alert(`从B站接口获取视频地址失败\ nresult: $ {
                                                                                        JSON.stringify(obj)
                                                                                    }\
                                                                                    n\ n点击确定, 进入设置页面关闭'使用B站接口获取视频地址'功能`, balh_ui_setting.show)
                                                                                } else {
                    ...
                    / / data有可能为 null
                                                                        if (data && data.code === -403) {
                                                                            ...
                                                                            if (alertWhenError) {
                                                                                ...
                                                                                n点击确定刷新界面`, window.location.reload.bind(window.location));
                                                                            } else {
                                                                                return Promise.reject(new AjaxException(`服务器错误: $ {
                                                                                    JSON.stringify(data)
                                                                                }`, data ? data.code : 0))
                                                                            }
                                                                ...
                                                                const playurl_by_kghost = new BilibiliApi({
                                                                    ...
                                                                    transToProxyUrl: function(originUrl, proxyHost) {
                                                                        return originUrl.replace(/^(https:)?(\/\/api\.bilibili\.com\/)/, `$1$ {
                                                                            proxyHost
                                                                        }`)
                                                                    },
                                                ...
                                                const balh_feature_runPing = function() {
                                                    ...
                                                    var pingResult = function() {
                                                        ...
                                                        testUrlIndex < testUrl.length ? pingLoop() : pingOutput.appendChild(_('a', {
                                                            href: 'javascript:',
                                                            event: {
                                                                    ...
                                                                    if (!isLogin()) {
                                                                        ...
                                                                        util_ui_pop({
                                                                            content: [
                                                                                ...
                                                                                n要不要考虑进行一下授权？\ n\ n授权后可以观看区域限定番剧的 1080P\ n（如果你是大会员或承包过这部番的话）\ n\ n你可以随时在设置中打开授权页面`)
                                                                            ],
                                                                            onConfirm: () => {
        ...
        window.addEventListener('
                                                            message ', function (e) {
            if (typeof e.data !== '
                                                            string ') return // 只处理e.data为string的情况
            switch (e.data.split(' : ')[0]) {
                case '
                                                            BiliPlus - Login - Success ': {
                    //登入
                ...
                case '
                                                            balh - login - credentials ': {
                    balh_auth_window.close();
                    let url = e.data.split(' : ')[1];
                    util_ui_popframe(url.replace('
                                                            http: //link.acg.tv/forum.php', balh_config.server + '/login'));
                                                            break;
                                                        }
                                                        }
                                                    })
                                                ...
                                                }, util_init.PRIORITY.DEFAULT, util_init.RUN_AT.DOM_LOADED_AFTER) return {
                                                    showLogin,
                                                    showLogout,
                                                    isLogin,
                                                    isLoginBiliBili,
                                                ...
                                                function generatePlayer(data, aid, page, cid) {
                                                        ...
                                                        className: 'main-inner'
                                                            ...
                                                            id: 'common_report'
                                                                ...
                                                                className: 'b-head'
                                                                    ...
                                                                    _('a', {
                                                                    className: 'del-log',
                                                                    href: ` //www.bilibili.com/replydeletelog?aid=${aid}&title=${data.title}`, target: '_blank' }, [_('text', '查看删除日志')])
                                                                ]),
                                                                    _('div', {
                                                                    className: 'comm',
                                                                    id: 'bbComment'
                                                    ...
                                                    }, util_init.PRIORITY.DEFAULT, util_init.RUN_AT.COMPLETE) return true // 随便返回一个值...
                                                    }())
                                                                                        ...
                                                                                        className: 'info-content'
                                                                                            ...
                                                                                            className: 'bangumi-info-r'
                                                                                                ...
                                                                                                className: 'info-count'
                                                                                                    ...
                                                                                                    }, [_('text', '总播放')]), _('em', {}, [_('text', data.result.play_count)])]),
                                                                                                    _('span', {
                                                                                                    className: 'info-count-item info-count-item-fans'
                                                                                                }, [_('span', {
                                                                                                        className: 'info-label'
                                                                                                    }, [_('text', '追番人数')]), _('em', {}, [_('text', data.result.favorites)])]),
                                                                                                    _('span', {
                                                                                                    className: 'info-count-item info-count-item-review'
                                                                                                }, [_('span', {
                                                                                                        className: 'info-label'
                                                                                                    }, [_('text', '弹幕总数')]), _('em', {}, [_('text', data.result.danmaku_count)])])
                                                                                            ]),
                                                ...
                                                const balh_ui_setting = (function() {
                                                    function addSettingsButton() {
                                                        ...
                                                        const createBtnStyle = (size, diffCss) => {
                                                            diffCss = diffCss || `#balh - settings - btn {
                                                                bottom: 110px;
                        ...
                        border-color: # 00a1d6;
                                                        ...
                                                        if (indexNav == null) {
                                                            ...
                                                            if (util_page.bangumi_md()) {
                                                                ...
                                                                indexNav.appendChild(createBtnStyle('44px', `#balh - settings - btn {
                                                                    float: left;
                                                                    margin: 3px 0 0 20px;
                                                                    background: #FFF;
                                                                    border - radius: 10px;
                                                            ...
                                                            if (indexNav) {
                                                                settingBtnSvgContainer = indexNav.appendChild(_('div', {
                                                                    ...
                                                                    event: {
                                                                        ...
                                                                        style: {
                                                                            ...
                                                                            style: {
                                                                                ...
                                                                                href: 'https://github.com/ipcjs/bilibili-helper/blob/user.js/bilibili_bangumi_area_limit_hack.md#大会员账号被b站永封了',
                                                                                target: '_blank'
                                                                            }, [_('text', '(？)')])]),
                                                                            _('label', {
                                                                            style: {
                                                                        ...
                                                                        event: {
                                                                            ...
                                                                            _('text', '　'),
                                                                            _('a', {
                                                                            href: 'https://github.com/ipcjs/bilibili-helper/blob/user.js/bilibili_bangumi_area_limit_hack.md',
                                                                            target: '_blank'
                                                                        }, [_('text', '帮助说明')]),
                                                                            _('text', '　'),
                                                                            _('a', {
